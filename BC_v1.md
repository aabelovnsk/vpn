...
4. **Переадресацию HTTP→HTTPS выключить**, пока `openssl` не покажет `DNS:cdn.vc01zbbxr.tech`. Иначе браузер сразу идёт на HTTPS со служебным сертификатом и пишет `net::ERR_CERT_COMMON_NAME_INVALID`.  
5. Подождать до **15 минут** после сохранения ресурса. Покажи в консоли ресурса, что сертификат именно CM и домен `cdn.vc01zbbxr.tech` (не origin и не apex).  
6. Когда SAN верный — включить переадресацию HTTP→HTTPS.
